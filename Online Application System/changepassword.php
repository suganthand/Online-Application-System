...
if(isset($_POST['currentpass'])) {
    ...
    $find_user = $username;
    $form_password = $_POST['currentpass'];
    $newpass = $_POST['newpass'];

    $find_pass = "SELECT salt, password, email FROM users WHERE email='$find_user'";
    ...
    if ($check_hash == $stored_hash)
    {
        ...
        $combo = $user_salt . $newpass;
        $hashedpass = hash('sha512', $combo);

        $update = "UPDATE users SET salt ='$user_salt', password ='$hashedpass' WHERE email='$username'";
    ...
    <script type="text/javascript">
        ...
        jQuery(function(){
            ...
            jQuery("#newpass").validate({
                expression: "if (VAL.match(/^(?=.*?[A-Z])(?=.*?[a-z])(?=.*?[0-9])(?=.*?[!@#$%^&*+_-])(?!.*\s).{8,15}$/) && VAL) return true; else return false;",
                message: "Please enter a valid password (8-15 alphanumeric characters, special characters: !@#$%^&*+_- only)"
            });
            jQuery("#newpassconfirm").validate({
                expression: "if ((VAL == jQuery('#newpass').val()) && VAL) return true; else return false;",
    ...
    <table width="865" border="0" cellpadding="0">
        ...
        <td class="main" colspan="2" valign="top">
            ...
            <div align="center">
                <br><br><br>
                <?php
                if($flag==1) echo "<h4 align=center style=color:green;> Password Changed Successfully !</h4>";
                ?>
                <form name="changepassword" action="changepassword.php" method="post" class="AdvancedForm">
                    <table align="center" cellpadding="3" cellspacing="2">
                        ...
                        <tr>
                            ...
                            <td>
                                <input type="password" name="currentpass" id="currentpass" size="50" required >
                            </td>
                        </tr>
                        <tr>
                            <td>
                            ...
                            <td>
                                <input type="password" name="newpass" id="newpass" size="50"  required>
                            </td>
                        </tr>
                        ...
                        <tr>
                            ...
                            <td>
                                <input type="password" name="newpassconfirm" id="newpassconfirm" size="50" required>
                            </td>
                        </tr>
                        <tr>
                            <td>
