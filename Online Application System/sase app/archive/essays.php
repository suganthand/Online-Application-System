...
if(isset($_FILES["acads"]["name"]))
{
    ...
    $acadsFile = $target_dir . $appid . '-acads.doc';
    $file = $appid . '-acads.doc';
    $file_size = $_FILES["acads"]["size"];
    ...
    if($acadsUploadOk)
    {
        if (move_uploaded_file($_FILES["acads"]["tmp_name"], $acadsFile))
        {
            ...
            $update = "UPDATE academic_uploads SET status=TRUE, file='$file', size='$file_size', type='$filetype'  WHERE applicant_id='$appid'";
        ...
        else
        {
            ...
            $update = "UPDATE academic_uploads SET status=FALSE, file='', size='', type=''  WHERE applicant_id='$appid'";
            $result = mysql_query($update) or die(mysql_error());
        }
    }
}

$select = "SELECT status, file FROM academic_uploads WHERE applicant_id='$appid'";
                    ...
                    <table align="center" cellpadding="3" cellspacing="2">
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                if($acadsUploaded==false && $acadsExtnInvalid==true)
                                {
                                    echo "<b><p style=color:red>Invalid extension! Only .doc files allowed!</p></b>";
                                }

                                if($acadsUploaded==false && $acadsTooLarge==true)
                                {
                                    echo "<b><p style=color:red>File too large! Limit to 500KB!</p></b>";
                                }

                                if($acadsUploaded==true)
                                {
                                    echo "<a href=\"uploads/$file\">View uploaded file</a>";
                                    echo "<input type='file' name='acads' id='acads'>
                                          <input type='submit' value='Replace' name='acadsUpload'>";
                                }

                                if($acadsUploaded==false && $status==1)
                                {
                                    echo "<a href=\"uploads/$file\">View uploaded file</a>";
                                    echo "<input type='file' name='acads' id='acads'>
                                          <input type='submit' value='Replace' name='acadsUpload'>";
                                ...
                                if($acadsUploaded==false && $status==0)
                                {   echo "<b><p style=color:red>No file uploaded!</p></b>";
                                    echo "<input type='file' name='acads' id='acads'>
                                          <input type='submit' value='Upload' name='acadsUpload'>";
                                }
                                ?>
