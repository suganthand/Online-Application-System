...
$insert = "UPDATE users SET app_status=1 WHERE email='$username'";
$result = mysql_query($insert) or die('Insertion error '.mysql_error());

$select = "SELECT id FROM users WHERE email='$username'";
$result = mysql_query($select) or die('App status error '.mysql_error());
...
$appid="";
$count = strlen($id);
if($count == 1) {$appid = "MSIT".$year."00".$temp;}
elseif($count == 2) {$appid = "MSIT".$year."0".$temp;}
elseif($count == 3) {$appid = "MSIT".$year.$temp;}
$update = "UPDATE users SET applicant_id='$appid' WHERE email='$username'";
$result = mysql_query($update) or die('App ID generation error '.mysql_error());
...
if(!$rows)
{
    $insert = "INSERT INTO education_details (applicant_id, email) VALUES('$appid','$username')";
...
if(!$rows)
{
    $insert = "INSERT INTO essays (applicant_id, email) VALUES('$appid','$username')";
    $result = mysql_query($insert) or die(mysql_error());
}

$select = "SELECT * FROM academic_uploads WHERE applicant_id='$appid'";
$result = mysql_query($select) or die(mysql_error());
$rows = mysql_num_rows($result);

if(!$rows)
{
    $insert = "INSERT INTO academic_uploads (applicant_id, email) VALUES('$appid','$username')";
    $result = mysql_query($insert) or die(mysql_error());
}

$select = "SELECT * FROM experience_uploads WHERE applicant_id='$appid'";
$result = mysql_query($select) or die(mysql_error());
$rows = mysql_num_rows($result);

if(!$rows)
{
$insert = "INSERT INTO experience_uploads (applicant_id, email) VALUES('$appid','$username')";
$result = mysql_query($insert) or die(mysql_error());
}

$select = "SELECT * FROM critical_essay WHERE applicant_id='$appid'";
$result = mysql_query($select) or die(mysql_error());
$rows = mysql_num_rows($result);

if(!$rows)
{
    $insert = "INSERT INTO critical_essay (applicant_id, email) VALUES('$appid','$username')";
    $result = mysql_query($insert) or die(mysql_error());
}
...
if(!$rows)
{
    $insert = "INSERT INTO document_uploads (applicant_id, email) VALUES('$appid','$username')";
    $result = mysql_query($insert) or die(mysql_error());
}
...
<div align="center">
    ...
    <table width="865" border="0" cellpadding="0">
        ...
        <td class="main" colspan="2" valign="top">
            ...
            <div align="center">
            <p> 1. Complete the form in all respects and attach necessary supplementary materials </p>
            <p> 2. Your application will not be processed unless you make the application fee payment online </p>
            </div>
        </td>
